...
import { TeachersRecordsRepository } from './teachers.repository.records';
...
export abstract class TeachersSessionsRepository extends TeachersRecordsRepository {
  async findNextSessionsByFormationIds(
    formationIds: string[],
    now: Date,
  ): Promise<
    Map<string, { id: string; startAt: Date; endAt: Date; roomCode: string }>
  > {
    const map = new Map<
      string,
      { id: string; startAt: Date; endAt: Date; roomCode: string }
    >();
    ...
    const rows = await this.db
      ...
      .where(
        and(
          ...
          gte(formationSessions.startAt, now),
        ),
      )
      .orderBy(
        asc(formationSessions.formationId),
        asc(formationSessions.startAt),
      );

    for (const r of rows) {
      if (!map.has(r.formationId)) {
  ...
  async findTeacherFormationSessions(
    teacherId: string,
    formationId: string,
